...
interface BlePeripheralChannel {
  ...
  fun startAdvertising(services: List<UUID>, serviceDatas: Map<UUID, ByteArray>, localName: String)
      ...
      run {
        ...
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            ...
            val serviceDatasArg = args[1] as Map<UUID, ByteArray>
            val localNameArg = args[2] as String
            var wrapped: List<Any?>
            try {
              api.startAdvertising(servicesArg, serviceDatasArg, localNameArg)
              wrapped = listOf<Any?>(null)
            } catch (exception: Throwable) {
              wrapped = wrapError(exception)
            }
